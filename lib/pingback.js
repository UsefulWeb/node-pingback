...
Pingback.send = (function() {
  var xml = [
    '<?xml version="1.0"?>',
    '<methodCall>',
    '  <methodName>__METHOD__</methodName>',
    '  <params>',
  ...
  return function(target, source, args, func) {
    var pingback, params = [], method = this._method;
    ...
    args.forEach(function(value) {
      params.push('    <param><value><string>__VALUE__</string></value></param>'.replace('__VALUE__', value));
    });
    ...
    request(target, function(err, res) {
      ...
      var body = xml
        ...
        .replace('__PARAMS__', params.join('\n'))
...
Pingback.scan = function(text, source, func) {
  ...
  text.replace(/href=([^\s>]+)/gi, function(__, target) {
    ...
    Pingback.send(target, source, null, func);
  });
};
